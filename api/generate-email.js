export default async function handler(req, res) {
  ...
  try {
    ...
    console.log('Generated Gemini prompt length:', geminiPrompt.length)
    ...
    if (parsed.subjects.length === 0 || parsed.bodies.length === 0) {
      console.error('Parsing failed:', parsed)
      return res.status(500).json({ 
        subjects: [], 
        bodies: [], 
...
function createGeminiPrompt(prompt) {
  const lengthGuide = {
    short: '50-100 words',
...
- Tone: ${prompt.tone} (${toneDescriptions[prompt.tone] || 'professional'})
- Call to Action: ${prompt.cta}
- Email Length: ${prompt.length} (${lengthGuide[prompt.length]})
...
5. All emails should be ${lengthGuide[prompt.length]} in length
